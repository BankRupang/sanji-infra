...
세션이 열린 채로 다른 터미널에서 접속합니다. RDS는 기본 파라미터 그룹이 SSL을 강제하므로 `sslmode=require`가 필요합니다.

```bash
# RDS (비밀번호는 아래 SSM 명령으로 조회)
...
dev/prod 구분 없이 동일하게 동작하며, 접근 통제는 SG가 아니라 모니터링 EC2에 SSM 세션을 열 수 있는 IAM 권한으로 합니다.
