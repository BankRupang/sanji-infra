...
aws ssm put-parameter --overwrite --type SecureString --region $REGION \
  --name /sanji/kafka/cluster-id --value "$CLUSTER_ID"

# Grafana 관리자 비밀번호
aws ssm put-parameter --overwrite --type SecureString --region $REGION \
  --name /sanji/monitoring/grafana-admin-password --value "원하는-비밀번호"

# Grafana 알림용 Slack Webhook (선택)
aws ssm put-parameter --overwrite --type SecureString --region $REGION \
  --name /sanji/monitoring/slack-webhook-url --value "https://hooks.slack.com/..."
```

> Kafka 사설 IP(`/sanji/kafka/private-ip`)는 Terraform이 자동으로 채워두므로
> 직접 등록할 필요가 없습니다.
...
(모니터링 EC2는 `docker-compose.monitoring.yml`)
...
1. 역할 ARN을 확인합니다.
   ...
   ```
2. GitHub Actions 워크플로우에서 이 역할을 assume 합니다.
   ```yaml
   permissions:
   ...
   steps:
     - uses: aws-actions/configure-aws-credentials@v4
       with:
         role-to-assume: <위에서 확인한 ARN>
         aws-region: ap-northeast-2
   ```
3. 이후 워크플로우에서 ECR push, ECS 배포(`aws ecs update-service --force-new-deployment`),
   EC2 배포(SSM Run Command)를 키 없이 할 수 있습니다.
...
1. [ ] `terraform.tfvars` 작성 (`db_password`, `admin_cidr`)
2. [ ] `terraform init`
3. [ ] `terraform apply`
4. [ ] SSM 시크릿 실제 값 채우기 (5-1, 5-2)
5. [ ] RDS 스키마 + pgvector 생성 (5-3)
6. [ ] 도커 이미지 ECR push (6단계, 또는 GitHub Actions)
7. [ ] Kafka / 모니터링 EC2 배포 (7단계)
8. [ ] `terraform output` 으로 접속 주소 확인
9. [ ] GitHub Actions 역할 연동 (8단계)
