...
모니터링 EC2를 통해 Keycloak에 sanjijk realm을 import하고 user-service 클라이언트 시크릿을 발급해 SSM(`/sanji/prod/keycloak/client-secret`)에 저장합니다.

전제 조건: Keycloak ECS 서비스가 정상 기동 중, SSM의 `/sanji/prod/keycloak/admin-password`가 실제 값으로 채워진 상태.
...
`terraform.tfvars` 에 값을 수정하고 `terraform apply` 만 다시 하면 됩니다.
...
1. [ ] `bootstrap/` 폴더에서 S3 버킷 + SSM 파라미터 생성: `cd bootstrap && terraform init && terraform apply`
...
7. [ ] RDS 스키마 초기화 확인 (keycloak/langfuse: terraform apply 시 자동, Spring 서비스: 각 서비스 기동 시 자동, pgvector: ai-service 기동 시 자동)
