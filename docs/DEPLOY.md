...
1단계(`bootstrap apply`)에서 시크릿 파라미터들을 **"CHANGE_ME"** 로 미리 만들어 두었습니다.
...
```bash
# Kafka EC2 (scripts/deploy-kafka.sh 실행)
cd /home/ec2-user/san-ji-jik-kyeng
git fetch origin && git reset --hard origin/main   # pull 대신 reset으로 충돌 방지
docker compose -f docker-compose.kafka.yml pull
docker compose -f docker-compose.kafka.yml up -d

# 모니터링 EC2 (scripts/deploy-monitoring.sh 실행)
git fetch origin && git reset --hard origin/main
# SSM / IMDS / RDS API에서 환경변수 읽어 .env 파일 생성
# prometheus.prod.yml.template에 Kafka IP를 채워 prometheus.prod.yml 생성
...
ECS 서비스 10개를 한 번에 배포하면 Fargate vCPU 한도(30)를 초과하기 때문에 4개 wave로 나눠 순서대로 배포합니다.

```
build → wave-1(config/discovery/gateway) → wave-2(user/auction/order/payment) → wave-3(bid/notification) → wave-4(ai)
```

wave 안에서는 서비스들이 동시에 배포됩니다. 특정 wave가 실패하면 Actions UI에서 그 wave만 재실행할 수 있습니다.

ai-service는 pgvector 스키마 초기화와 Gemini API 연결로 기동 시간이 길어 단독 wave로 분리되어 있습니다. (최대 대기 30분)

> 다른 저장소 이름을 쓰면 `terraform.tfvars` 의 `github_repo` 값을 맞춰 바꾼 뒤 `terraform apply` 하세요.

---

## 9. 사양/대수 바꾸기 (variable로 조정)

부하테스트 결과에 따라 사양을 코드 한 줄로 조정할 수 있습니다.
...
1. [ ] `bootstrap/` 폴더에서 S3 버킷 + SSM 파라미터 14개 생성: `cd bootstrap && terraform init && terraform apply`
